        ...
        return (
          <div key={index} data-open={isOpen ? '' : undefined} className={tm(accordionItemStyles({ variant, radius, withSeparator: withSeparator && !isLast }), spaced === 'none' && 'not-last:rounded-b-none not-first:rounded-t-none')}>
            <button type='button' aria-expanded={isOpen} className={accordionItemHeaderStyles({ size, variant })} onClick={() => handleToggleCollapse(index)}>
              {item.icon && <span className='flex shrink-0 items-center text-gray-500 dark:text-gray-400'>{item.icon}</span>}

              <div className='flex flex-1 flex-col items-start text-left'>
                <p className='font-medium leading-snug text-gray-800 dark:text-gray-200'>{item.title}</p>
                {item.description && <span className='mt-0.5 text-gray-500 dark:text-gray-400'>{item.description}</span>}
              </div>

              <span aria-hidden='true' className={tm('flex shrink-0 items-center text-gray-500 transition-transform duration-200 dark:text-gray-400', isOpen ? 'rotate-90' : 'rotate-0')}>
            ...
            <div role='region' className={tm('grid transition-all duration-300', isOpen ? 'grid-rows-[1fr]' : 'grid-rows-[0fr]')}>
              <div className='overflow-hidden'>
                <div className='p-3 text-sm text-gray-700 dark:text-gray-300'>{item.content}</div>
              </div>
            </div>
          </div>
        )
...
const accordionItemStyles = cva(['group/item overflow-hidden'], {
  variants: {
    variant: {
      ...
      filled: 'border border-gray-300 dark:border-gray-700',
    ...
    withSeparator: {
      true: 'border-b border-gray-300 dark:border-gray-700',
      false: '',
    },
  },
  defaultVariants: {
...
const accordionItemHeaderStyles = cva(['w-full flex items-center cursor-pointer p-3', 'rounded-[inherit] group-data-open/item:rounded-b-none', 'focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gray-400'], {
  variants: {
    variant: {
      default: 'bg-transparent hover:bg-gray-100/60 dark:hover:bg-gray-800/60',
      solid: 'bg-gray-100 hover:bg-gray-200/70 dark:bg-gray-800 dark:hover:bg-gray-700/70',
      filled: 'bg-gray-100/50 hover:bg-gray-100 dark:bg-gray-800 dark:hover:bg-gray-700/60',
      outline: 'bg-transparent hover:bg-gray-50 dark:hover:bg-gray-900/40',
    },
    size: {
